...
class FW_Db_Options_Model_Settings extends FW_Db_Options_Model {
	...
	protected function set_values($item_id, $values, array $extra_data = array()) {
		FW_WP_Option::set('fw_theme_settings_options:' . fw()->theme->manifest->get_id(), $values, null);
	}
	...
	protected function _init() {
		 ...
		 * @param mixed $value
		 */
		function fw_set_db_settings_option( $value, $option_id = null ) {
			FW_Db_Options_Model_Settings::_get_instance('settings')->set($value, null, $option_id);
		}
	}
}
...
class FW_Db_Options_Model_Post extends FW_Db_Options_Model {
	...
	protected function _init() {
		 ...
		 * @param $value
		 */
		function fw_set_db_post_option( $value, $post_id = null, $option_id = null ) {
			FW_Db_Options_Model::_get_instance('post')->set($value, intval($post_id), $option_id);
		}
...
class FW_Db_Options_Model_Term extends FW_Db_Options_Model {
	...
	protected function _init() {
		 ...
		 * @return null
		 */
		function fw_set_db_term_option( $term_id, $taxonomy, $value, $option_id = null ) {
			if ( ! taxonomy_exists( $taxonomy ) ) {
				return null;
			}

			FW_Db_Options_Model::_get_instance('term')->set($value, intval($term_id), $option_id, array(
				'taxonomy' => $taxonomy
			));
		}
...
class FW_Db_Options_Model_Extension extends FW_Db_Options_Model {
	...
	protected function set_values($item_id, $values, array $extra_data = array()) {
		FW_WP_Option::set( 'fw_ext_settings_options:' . $item_id, $values, null );
	}
	...
	protected function _init() {
		 ...
		 * @param mixed $value
		 */
		function fw_set_db_ext_settings_option( $extension_name, $value, $option_id = null ) {
			...
			FW_Db_Options_Model::_get_instance('extension')->set($value, $extension_name, $option_id);
		}
	}
}
...
class FW_Db_Options_Model_Customizer extends FW_Db_Options_Model {
	...
	protected function _init() {
		 ...
		 * @param mixed $value
		 */
		function fw_set_db_customizer_option( $value, $option_id = null ) {
			FW_Db_Options_Model::_get_instance('customizer')->set($value, null, $option_id);
		}
 ...
 */
{
	 ...
	 * @param mixed $value
	 */
	function fw_set_db_extension_data( $extension_name, $value, $multi_key = null ) {
		if ( ! fw()->extensions->get( $extension_name ) ) {
			trigger_error( 'Invalid extension: ' . $extension_name, E_USER_WARNING );
		...
		FW_WP_Option::set( 'fw_extensions', $value, $multi_key );
	}
}
