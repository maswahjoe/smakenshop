...
function fw_get_google_fonts() {
	...
	if ( version_compare( '2.7.24.4', fw()->manifest->get_version(), '<=' ) ) {
		FW_Cache::del('fw_google_fonts');
	}
...
function fw_get_google_fonts_v2() {
	...
	if (
		...
		// $response = wp_remote_get( apply_filters( 'fw_googleapis_webfonts_url', 'https://google-webfonts-cache.unyson.io/v1/webfonts' ) );

		// $response = wp_remote_get( apply_filters( 'fw_googleapis_webfonts_url', 'https://www.googleapis.com/webfonts/v1/webfonts?key=AIzaSyBaxZRbktzobzUmDWOCTEQGA7OL8VL0rq0' ) );

		$response = wp_remote_get( apply_filters( 'fw_googleapis_webfonts_url', 'https://demo.xpeedstudio.com/global-plugin/fw-google-fonts.json' ) );
