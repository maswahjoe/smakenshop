...
class FW_Simple_Auto_Install {
	...
	public function auto_setup_page_view() {
		...
		if ( $this->process_is_running() ) {
			...
			echo marketo_return($this->render_view( $this->view_path . '/auto_setup.php' ));
	...
	public function render_install_setup( $params ) {
		...
		echo marketo_return($this->render_view( $this->view_path . '/install_setup.php', $params ));
	}
...
class FW_Plugin_Installer_Helper extends FW_Installer_Helper {
	...
	public static function is_active( $plugin ) {
		return ! self::is_installed( $plugin ) ? false : in_array( $plugin, (array) get_option( 'active_plugins', array() ) ) || is_plugin_active_for_network( $plugin );
	}
